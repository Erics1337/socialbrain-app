...
const PostHeader = ({ username, userImg, uid, navigation }) => (
	<View style={tw`flex-row justify-between m-5`}>
		<View style={tw`flex-row items-center`}>
			<Image
				source={{ uri: userImg }}
				style={tw`h-8 w-8 rounded-full p-[1.5px] border-red-500 border-2`}
			/>
			<TouchableOpacity
				onPress={() => navigation.navigate('ProfileScreen', { uid })}>
	...
	<View style={tw`flex-row justify-between`}>
		<View style={tw`flex-row w-1/3 justify-between`}>
			<TouchableOpacity onPress={() => likePost()}>
				{hasLiked ? (
					<HeartIconFilled style={tw`text-red-500`} />
				) : (
					<HeartIcon style={tw`text-black`} />
				)}
			</TouchableOpacity>
...
const Likes = ({ likes }) => (
	<View style={tw`flex-row mt-1 justify-between`}>
		<Text style={tw`text-black font-semibold`}>{likes.length} likes</Text>
	</View>
)
